...
module.exports = function erode(worker, data) {
    ...
    return q.Promise(function(resolve, reject) {
        try {
            ...
            var changedBorderCells = [];
            if (worker.terrain._bounds) {
                worker.terrain._bounds.forEach(function(borderCell) {
                    if (borderCell.postBounds()) {
                        changedBorderCells.push(borderCell);
                    }
                });
            }
            if (changedBorderCells.length) {
                worker.sendBorderCellUpdate()
                    .then(worker.getBorderCellUpdates())
                    .then(resolve, reject);
            } else {
                worker.getBorderCellUpdates()
                    .then(resolve, reject);
            }
        } catch (err) {
