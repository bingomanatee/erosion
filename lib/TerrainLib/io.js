...
module.exports = {
    toJSON: function (withData, withBounds) {
        var obj = _.pick(this, ['iSize', 'iStart', 'jSize', 'jStart']);
        if (withData) {
            obj.data = this.to2Darray();
        }

        if (this._bounds && withBounds){
            obj.bounds = [];

            console.log("bounds:", this._bounds);

            this._bounds.forEach(function(cell){
                obj.bounds.push(cell.toJSON());
            });
        }
        return obj;
    },
    ...
    toData: function (full) {
        var size = DIMENSION_SIZE // sizing parameters
          + 4 * this._cells.length;
        var buffer = new SmartBuffer(size, CONST.encoding);
        ...
        _.each(this._cells, function (cell) {
            cell.rawDataToBuffer(buffer, full);
        });
