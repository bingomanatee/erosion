...
Terrain.prototype = _.extend({
    ...
    neighbors: function (i, j, omitCenter) {
        if (i) {
            var cell = this._cells[i * this.jSize + j];
        }
    },
    ...
    _getCell: function (ii, jj, checkBounds) {
        ...
        if (i >= 0
          ...
          && j < this.jSize) {
            ...
            if (checkBounds) {
                for (var n = 0; n < this._bounds.length; ++n) {
                    c = this._bounds[n];
                    if (c.i == ii && c.j === jj) {
                        return c;
                    }
            ...
            throw new Error(['out of range getHeight', i, j, 'from', JSON.stringify(this.toJSON())].join(' '));
        }
    },
