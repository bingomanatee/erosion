...
Terrain.prototype = _.extend({
        ...
        setBounds: function (bounds) {
            this._boundsIndex = bounds.reduce(function (out, bound) {
                if (this.contains(bound.i, bound.j)) {
                    console.log('excluding bounds cell', bound.toString(), 'from', this.toString());
                } else if (this.contains(bounds.i, bounds.j, 1)){
                    console.log('including bounds cell', bound.toString(), 'from', this.toString());
                    out[bound.i + ',' + bound.j] = bound;
                } else {
                    console.log('excluding distant bounds cell', bound.toString(), 'from', this.toString());
                }
                return out;
            }.bind(this), {});
            ...
            console.log('terrain:', this.toString(), 'bounds:', bounds);
        },
